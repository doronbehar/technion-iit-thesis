\chapter{Simulation Software Manual \& Technical Details}\label{chap:software}
...
\texttt{./sim.py} is basically built with a command line option per parameter, enabling you to choose which parameters to scan. Any command line option not specified will prompt you to choose the values to iterate over interactively, or a default value will be chosen.
...
When you run \texttt{./sim.py} and you want to choose specific values of a certain float like parameter, you use the indices of the array's values defined in \texttt{PARAMETERS\_INFO}. For example to scan the initial temperatures $[10,40]\mathrm{K}$, you would use \texttt{--T\_init 1 3}. If you want to specify a certain range of values, you can use Bash shell syntax to choose e.g $[5,10,20]\mathrm{K}$ with \texttt{--T\_init \{0..2\}}, which simply is expanded by Bash to \texttt{--T\_init 0 1 2}. If the option \texttt{--T\_init} is not specified at all, you will be prompted to pick the temperatures you wish to simulate over.
...
\begin{itemize}% \setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}\setlength{\parsep}{0pt}
    ...
    \item \texttt{--rf-divisor}: How many timesteps to put in a single RF cycle (default: [1597]), see also section~\ref{sec:theory/convergence}.
...
This command tests the effect of the RF divisors on the stability and convergence (see \ref{sec:theory/convergence}). Note that \texttt{--seed \{90..93\}} is expanded by the shell and eventually interpreted as \texttt{--seed 90 91 92 93}, which means: Run each simulation with 4 different initial conditions defined by the seeds $90-93$. Using a constant set of seeds, and not \texttt{-seed=-4}, makes sure that every process initiated by GNU \texttt{parallel} will use the same 4 initial conditions, and not 4 randomly picked initial condition seeds (picked by each \texttt{./sim.py} process).
...
Another common scenario where \texttt{./sim-continue.py} is very useful, is running part of a simulation from a certain point in time, but with a higher RF divisor. You can observe example results where this is needed in section~\ref{sec:theory/convergence}. The relevant self-explanatory command line options are \texttt{--rf-divisor} and \texttt{--from}. The argument to \texttt{--from} is a time point in $\mathrm{ms}$, rounded downwards to the nearest time point found.
...
Another likely scenario one may encounter when researching, is best explained in the following example: Assume you ran the scans marked in table~\ref{tbl:sim-reconcile-example}, using e.g a 2 dimensional scan of parameters $(A, B) \in \{a_1, a_2\}\times\{b_1, b_2\}$, and then a 1 dimensional scan of $B \in \{b_2, b_3\}$ with $A = a_3$. You may suspect there's some coupling between parameters $A$ and $B$, and you want to get a full picture of the dependence. Therefore you wish to simulate the missing configurations, marked with $\times$ in table~\ref{tbl:sim-reconcile-example}.
...
\begin{itemize}
	...
	\item Saving the plot in any format supported by Matplotlib is possible using \texttt{--save-fig}, with the format specified by the given file's extension. If '\texttt{-}' is used as a file path, the contents of a PDF file containing the plot are printed to \texttt{stdout}, useful for piping it straight to the clipboard.
  	\item \texttt{--wfrac} Takes the intended LaTeX \texttt{\textbackslash linewidth} fraction used when including the saved figure.
	\item \texttt{--hprop} Takes the height/width proportion to use in the generated figure.
	\item \texttt{--always-show}: If saving, open the figure before finishing.
	\item \texttt{--legend}: When saving, put legend in the specified location. Use 'none' to disable legend altogether.
	\item \texttt{--legend-columns}: Specify the amount of legend columns to use. Relevant only when saving, and when using a legend location outside and '{upper,lower}'. The default value, is picked by the maximal input datasets' dimensions.
	...
	\item \texttt{--mass-conf} allows you to reduce the results dataset by performing simple operations upon the masses. One option is to use an argument of the form \texttt{m=AMU} that selects the given mass. A 2nd option is performing a mathematical operation between the light v.s heavy mass. The special argument \texttt{a-h-l} is useful for \texttt{-y E\_c} only, for showing the Coulomb interaction energy between the species. Using this option reduces the amount of displayers required to display all dimensions on the axes.
	\item \texttt{--smooth} smooths the time dependent data before analyzing it, using Scipy's \texttt{make\_smoothing\_spline}\footnote{See \url{https://docs.scipy.org/doc/scipy/reference/generated/scipy.interpolate.make_smoothing_spline.html}.} and given a $\lambda$ parameter.
\end{itemize}
...
\hline
$E_c$ & Coulomb energies\footnote{See also section~\ref{ssec:h5doctor_coulomb}.} [eV] & \makecell[tl]{\texttt{--part}} \\
\end{tabularx}
...
\subsection{Bottom Left: Scan's Dimensions Sliders}
...
\begin{itemize}
	...
	\item \texttt{--positions-label}: Choose a different positions label scheme. Defaults to \texttt{full}, which means show what masses are trapped, and how many of them are trapped at any given time. Other options are self explanatory: \texttt{only-mass}, \texttt{none}.
	\item \texttt{--separate-positions-figure}: Put the positions' axes in a separate figure. Useful for saving images of the 3D particles (projected onto 2D), in selected points in time
\end{itemize}
...
\section{\texttt{histogram-plot.py}}

To plot the histograms in figure~\ref{fig:histograms-Ti-comparison}, this script can be used. It acts similarly to \texttt{time-plot.py} and \texttt{plot.py} with regards to handling of multi-dimensional scans, but it includes one special command line option explained below.

To generate histograms with enough statistics and fine bins spacings, one has to take kinetic energies of the particles over many time steps. The script is hard-coded to divide the stabilization time period to $2$ segments, and the cooling time period to $10$ more segments. These segments are termed \emph{time windows} by the script.

By default, the last cooling time window is used, but multiple time windows can be specified by the \texttt{--time-windows} option, using indices starting at 0, ending at 11. When more then 1 time window is specified, this dimension is added to the plot, and colors / markers / linestyles can be used to vary the time windows, just like in section~\ref{ssec:manual/displayers-dimensions}.
...
\hline
\texttt{coulomb.py}             & Implement Coulomb energies calculation, using Freud'sA\cite{freud} \texttt{locality.AABBQuery} - allowing to iterate all nearest-neighbors efficiently. & Can calculate \& plot data like presented in section~\ref{sec:theory/init-coulomb} and in appendix~\ref{chap:coulomb}.                                                                                                                                      \\
\hline
...
\hline
\texttt{mathieu.py}             & Implement a \texttt{Trap} class solving the Mathieu equations given, as described in section~\ref{sec:theory/mathieu-inversly}.                                  & Can open figures \ref{fig:pure-mathieu-coefficients}, \ref{fig:pure-mathieu-fft}, \ref{fig:mathieu-stability} \& \ref{fig:mathieu-mass-dep}.Is also capable of exploring the dependence of $\{(a_i, q_i)\}$ solutions given $\beta_i$ in other ways. \\
\hline
\texttt{optimal\_cooling.py}    & None for simulating and plotting.                                                                                                                      & Calculates a theory that was not fully verified in this work, and wasn't important enough, and hence was left out.                                                                                                                                                                                               \\
\hline
\texttt{physical\_constants.py} & Put all physical and technical parameters in the same place.                                                                                           & Does nothing.                                                                                                                                                                                                                                        \\
\hline
\texttt{pwlfWrapper.py}         & Wraps the PWLF algorithm to ease handling of the fit parameters' uncertainties~\cite{uncertainties}.                                                                        & Does nothing.                                                                                                                                                                                                                                        \\
\hline
\texttt{bolg.py}                & Implements an algorithm (written by Claude.ai), that performs an opposite of file path globbing (hence the name). Used for picking figures' titles.    & Tests the algorithm using simply file paths arguments.                                                                                                                                                                                               \\
\end{tabularx}
